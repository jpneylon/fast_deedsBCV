...
void dataCostCL(unsigned long* data,unsigned long* data2,float* results,int m,int n,int o,int len2,int step1,int hw,float quant,float alpha,int randnum)
{
    ...
    if (accel) {
        ...
        if (time) {
            ...
            cout<<"\n==================================================\n";
            cout<<"Time for cuda_cost: "<<timeCOST<<" secs\n";
            }
    } else {
            ...
            for(int x=0;x<n1;x++){
                for(int y=0;y<m1;y++){
                    ...
                    for(int l=0;l<len2;l++){
                        ...
                        int idx = (y+x*m1+z*m1*n1)*len2+l;
                        results[idx]=out1*alpha1;                    
                    }
                }
            }
        }
...
void warpImageCL(float* warped,float* im1,float* im1b,float* u1,float* v1,float* w1){
    ...
    if (accel) {
        ...
        if (time) {
            ...
            float timeCOST=time2.tv_sec+time2.tv_usec/1e6-(time1.tv_sec+time1.tv_usec/1e6);
            cout<<"\n==================================================\n";
            cout<<"Time for cuda_warpImageCL: "<<timeCOST<<" secs\n";
        }
    } else {
    
        interp3(warped,im1,u1,v1,w1,m,n,o,m,n,o,true,accel);
...
void warpAffine(float* warped,float* input,float* im1b,float* X,float* u1,float* v1,float* w1){
    ...
    if (accel) {
        ...
        if (time) {
            ...
            float timeCOST=time2.tv_sec+time2.tv_usec/1e6-(time1.tv_sec+time1.tv_usec/1e6);
            cout<<"\n==================================================\n";
            cout<<"Time for cuda_warpAffine: "<<timeCOST<<" secs\n";
        }
    } else {
        for(int k=0;k<o;k++){
            for(int j=0;j<n;j++){
